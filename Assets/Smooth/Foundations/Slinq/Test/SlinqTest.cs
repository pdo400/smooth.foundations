...
namespace Smooth.Slinq.Test {
	...
	public class SlinqTest : MonoBehaviour {
		public static readonly DelegateFunc<Stuple<int, int>, Stuple<int, int>, bool> eq = (a, b) => a == b;
		public static readonly DelegateFunc<Stuple<int, int, int>, Stuple<int, int, int>, bool> eq_t3 = (a, b) => a == b;

		public static readonly DelegateFunc<Stuple<int, int>, int> to_1 = t => t._1;
		public static readonly Func<Stuple<int, int>, int> to_1f = t => t._1;

		public static readonly IEqualityComparer<Stuple<int, int>> eq_1 = new Equals_1<int, int>();
		//public static readonly DelegateFunc<Stuple<int, int>, Stuple<int, int>, bool> eq_1_p = Comparisons<Stuple<int, int>>.ToPredicate(eq_1);

		public static readonly StringBuilder stringBuilder = new StringBuilder();

		public static List<Stuple<int, int>> tuples1 = new List<Stuple<int, int>>();
		public static List<Stuple<int, int>> tuples2 = new List<Stuple<int, int>>();
		...
		private void Start() {
			tuples1 = new List<Stuple<int, int>>(maxCount);
			tuples2 = new List<Stuple<int, int>>(maxCount);
			loops = speedLoops;
		...
		private void LateUpdate() {
			...
			for (int i = 0; i < count; ++i) {
				tuples1.Add(new Stuple<int, int>(UnityEngine.Random.Range(minValue, maxValue + 1), i));
				tuples2.Add(new Stuple<int, int>(UnityEngine.Random.Range(minValue, maxValue + 1), i));
			}
		}

		private void TestCorrectness() {
			var testStuple = tuples2.Slinq().FirstOrDefault();
			var testInt = testStuple._1;
			...
			if (tuples1.Slinq().Contains(testStuple, eq_1) ^ tuples1.Contains(testStuple, eq_1)) {
			...
			if (!new List<Stuple<int, int>>[] { tuples1, tuples2 }.Slinq().Select(x => x.Slinq()).Flatten().SequenceEqual(tuples1.Concat(tuples2).Slinq(), eq)) {
			...
			if (!tuples1.Slinq().GroupJoin(tuples2.Slinq(), to_1, to_1, (a, bs) => Stuple.Create(a._1, a._2, bs.Count()))
			    .SequenceEqual(tuples1.GroupJoin(tuples2, to_1f, to_1f, (a, bs) => Stuple.Create(a._1, a._2, bs.Count())).Slinq(), eq_t3)) {
				Debug.LogError("GroupJoin failed.");
				testCorrectness = false;
			}
			
			if (!tuples1.Slinq().Join(tuples2.Slinq(), to_1, to_1, (a, b) => Stuple.Create(a._1, a._2, b._2))
			    .SequenceEqual(tuples1.Join(tuples2, to_1f, to_1f, (a, b) => Stuple.Create(a._1, a._2, b._2)).Slinq(), eq_t3)) {
			...
			if (!new List<Stuple<int, int>>[] { tuples1, tuples2 }.Slinq().SelectMany(x => x.Slinq()).SequenceEqual(tuples1.Concat(tuples2).Slinq(), eq)) {
			...
			if (!tuples1.Slinq().Zip(tuples2.Slinq()).SequenceEqual(
				...
				.Select(x => Stuple.Create(tuples1[x], tuples2[x])))) {
				Debug.LogError("Zip tuples failed.");
				testCorrectness = false;
			}

			if (!tuples1.Slinq().Zip(tuples2.Slinq(), (a, b) => Stuple.Create(a._1, b._1)).SequenceEqual(
				Slinqable.Sequence(0, 1)
				.TakeWhile(x => x < tuples1.Count && x < tuples2.Count)
				.Select(x => Stuple.Create(tuples1[x]._1, tuples2[x]._1)), eq)) {
			...
			if (!tuples1.Slinq()
			    ...
			    .SequenceEqual(Slinqable.Sequence(0, 1)
			               .TakeWhile(x => x < tuples1.Count || x < tuples2.Count)
			               .Select(x => Stuple.Create(
				x < tuples1.Count ?	new Option<Stuple<int, int>>(tuples1[x]) : new Option<Stuple<int, int>>(),
				x < tuples2.Count ?	new Option<Stuple<int, int>>(tuples2[x]) : new Option<Stuple<int, int>>())))) {
			...
			if (!tuples1.Slinq().Skip(midSkip)
			    ...
			    .SequenceEqual(Slinqable.Sequence(0, 1)
			               ...
			               .Select(x => Stuple.Create(
				x + midSkip < tuples1.Count ? new Option<Stuple<int, int>>(tuples1[x + midSkip]) : new Option<Stuple<int, int>>(),
				x < tuples2.Count ? new Option<Stuple<int, int>>(tuples2[x]) : new Option<Stuple<int, int>>())))) {
				Debug.LogError("ZipAll tuples failed.");
				testCorrectness = false;
			}
			...
			if (!tuples1.Slinq()
			    ...
			    .SequenceEqual(Slinqable.Sequence(0, 1)
			               ...
			               .Select(x => Stuple.Create(
				x < tuples1.Count ? new Option<Stuple<int, int>>(tuples1[x]) : new Option<Stuple<int, int>>(),
				x + midSkip < tuples2.Count ? new Option<Stuple<int, int>>(tuples2[x + midSkip]) : new Option<Stuple<int, int>>())))) {
				Debug.LogError("ZipAll tuples failed.");
				testCorrectness = false;
			}
			
			if (!tuples1.Slinq()
			    .ZipAll(tuples2.Slinq(), (a, b) => Stuple.Create(a, b))
			    .SequenceEqual(Slinqable.Sequence(0, 1)
			               .TakeWhile(x => x < tuples1.Count || x < tuples2.Count)
			               .Select(x => Stuple.Create(
				x < tuples1.Count ?	new Option<Stuple<int, int>>(tuples1[x]) : new Option<Stuple<int, int>>(),
			...
			if (!tuples1.Slinq()
			    .ZipAll(tuples2.Slinq().Skip(midSkip), (a, b) => Stuple.Create(a, b))
			    .SequenceEqual(Slinqable.Sequence(0, 1)
			               .TakeWhile(x => x < tuples1.Count || x + midSkip < tuples2.Count)
			               .Select(x => Stuple.Create(
				x < tuples1.Count ? new Option<Stuple<int, int>>(tuples1[x]) : new Option<Stuple<int, int>>(),
		...
		private List<Stuple<int, int>> RemovableList() {
			return new List<Stuple<int, int>>(tuples1);
		}

		private LinkedList<Stuple<int, int>> RemovableLinkedList() {
			return new LinkedList<Stuple<int, int>>(tuples1);
		}

		public class Equals_1<T1, T2> : IEquatable<Equals_1<T1, T2>>, IEqualityComparer<Stuple<T1, T2>> {
			...
			public bool Equals(Stuple<T1, T2> a, Stuple<T1, T2> b) {
				return equalityComparer.Equals(a._1, b._1);
			}

			public int GetHashCode(Stuple<T1, T2> a) {
