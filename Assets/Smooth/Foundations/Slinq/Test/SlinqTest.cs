...
namespace Smooth.Slinq.Test {
	...
	public class SlinqTest : MonoBehaviour {
		public static readonly DelegateFunc<Smooth.Algebraics.Tuple<int, int>, Smooth.Algebraics.Tuple<int, int>, bool> eq = (a, b) => a == b;
		public static readonly DelegateFunc<Smooth.Algebraics.Tuple<int, int, int>, Smooth.Algebraics.Tuple<int, int, int>, bool> eq_t3 = (a, b) => a == b;

		public static readonly DelegateFunc<Smooth.Algebraics.Tuple<int, int>, int> to_1 = t => t._1;
		public static readonly Func<Smooth.Algebraics.Tuple<int, int>, int> to_1f = t => t._1;

		public static readonly IEqualityComparer<Smooth.Algebraics.Tuple<int, int>> eq_1 = new Equals_1<int, int>();
		//public static readonly DelegateFunc<Smooth.Algebraics.Tuple<int, int>, Smooth.Algebraics.Tuple<int, int>, bool> eq_1_p = Comparisons<Smooth.Algebraics.Tuple<int, int>>.ToPredicate(eq_1);
		...
		private void TestCorrectness() {
			...
			if (!new List<Smooth.Algebraics.Tuple<int, int>>[] { tuples1, tuples2 }.Slinq().Select(x => x.Slinq()).Flatten().SequenceEqual(tuples1.Concat(tuples2).Slinq(), eq)) {
			...
			if (!tuples1.Slinq().GroupJoin(tuples2.Slinq(), to_1, to_1, (a, bs) => Smooth.Algebraics.Tuple.Create(a._1, a._2, bs.Count()))
			    .SequenceEqual(tuples1.GroupJoin(tuples2, to_1f, to_1f, (a, bs) => Smooth.Algebraics.Tuple.Create(a._1, a._2, bs.Count())).Slinq(), eq_t3)) {
				Debug.LogError("GroupJoin failed.");
				testCorrectness = false;
			}
			
			if (!tuples1.Slinq().Join(tuples2.Slinq(), to_1, to_1, (a, b) => Smooth.Algebraics.Tuple.Create(a._1, a._2, b._2))
			    .SequenceEqual(tuples1.Join(tuples2, to_1f, to_1f, (a, b) => Smooth.Algebraics.Tuple.Create(a._1, a._2, b._2)).Slinq(), eq_t3)) {
			...
			if (!new List<Smooth.Algebraics.Tuple<int, int>>[] { tuples1, tuples2 }.Slinq().SelectMany(x => x.Slinq()).SequenceEqual(tuples1.Concat(tuples2).Slinq(), eq)) {
			...
			if (!tuples1.Slinq().Zip(tuples2.Slinq()).SequenceEqual(
				...
				.Select(x => Smooth.Algebraics.Tuple.Create(tuples1[x], tuples2[x])))) {
				Debug.LogError("Zip tuples failed.");
				testCorrectness = false;
			}

			if (!tuples1.Slinq().Zip(tuples2.Slinq(), (a, b) => Smooth.Algebraics.Tuple.Create(a._1, b._1)).SequenceEqual(
				Slinqable.Sequence(0, 1)
				.TakeWhile(x => x < tuples1.Count && x < tuples2.Count)
				.Select(x => Smooth.Algebraics.Tuple.Create(tuples1[x]._1, tuples2[x]._1)), eq)) {
			...
			if (!tuples1.Slinq()
			    ...
			    .SequenceEqual(Slinqable.Sequence(0, 1)
			               .TakeWhile(x => x < tuples1.Count || x < tuples2.Count)
			               .Select(x => Smooth.Algebraics.Tuple.Create(
				x < tuples1.Count ?	new Option<Smooth.Algebraics.Tuple<int, int>>(tuples1[x]) : new Option<Smooth.Algebraics.Tuple<int, int>>(),
				x < tuples2.Count ?	new Option<Smooth.Algebraics.Tuple<int, int>>(tuples2[x]) : new Option<Smooth.Algebraics.Tuple<int, int>>())))) {
			...
			if (!tuples1.Slinq().Skip(midSkip)
			    ...
			    .SequenceEqual(Slinqable.Sequence(0, 1)
			               ...
			               .Select(x => Smooth.Algebraics.Tuple.Create(
				x + midSkip < tuples1.Count ? new Option<Smooth.Algebraics.Tuple<int, int>>(tuples1[x + midSkip]) : new Option<Smooth.Algebraics.Tuple<int, int>>(),
				x < tuples2.Count ? new Option<Smooth.Algebraics.Tuple<int, int>>(tuples2[x]) : new Option<Smooth.Algebraics.Tuple<int, int>>())))) {
				Debug.LogError("ZipAll tuples failed.");
				testCorrectness = false;
			}
			...
			if (!tuples1.Slinq()
			    ...
			    .SequenceEqual(Slinqable.Sequence(0, 1)
			               ...
			               .Select(x => Smooth.Algebraics.Tuple.Create(
				x < tuples1.Count ? new Option<Smooth.Algebraics.Tuple<int, int>>(tuples1[x]) : new Option<Smooth.Algebraics.Tuple<int, int>>(),
				x + midSkip < tuples2.Count ? new Option<Smooth.Algebraics.Tuple<int, int>>(tuples2[x + midSkip]) : new Option<Smooth.Algebraics.Tuple<int, int>>())))) {
				Debug.LogError("ZipAll tuples failed.");
				testCorrectness = false;
			}
			
			if (!tuples1.Slinq()
			    .ZipAll(tuples2.Slinq(), (a, b) => Smooth.Algebraics.Tuple.Create(a, b))
			    .SequenceEqual(Slinqable.Sequence(0, 1)
			               .TakeWhile(x => x < tuples1.Count || x < tuples2.Count)
			               .Select(x => Smooth.Algebraics.Tuple.Create(
				x < tuples1.Count ?	new Option<Smooth.Algebraics.Tuple<int, int>>(tuples1[x]) : new Option<Smooth.Algebraics.Tuple<int, int>>(),
			...
			if (!tuples1.Slinq()
			    .ZipAll(tuples2.Slinq().Skip(midSkip), (a, b) => Smooth.Algebraics.Tuple.Create(a, b))
			    .SequenceEqual(Slinqable.Sequence(0, 1)
			               .TakeWhile(x => x < tuples1.Count || x + midSkip < tuples2.Count)
			               .Select(x => Smooth.Algebraics.Tuple.Create(
				x < tuples1.Count ? new Option<Smooth.Algebraics.Tuple<int, int>>(tuples1[x]) : new Option<Smooth.Algebraics.Tuple<int, int>>(),
